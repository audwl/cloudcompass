...
export default function Dashboard() {
  ...
  return <main>
    ...
    <section className="discovery" id="discovery"><div className="shell">
      ...
      <div className="profileInsight"><span>✦</span><div><small>INITIAL ASSESSMENT</small><b>{workload} on {provider}</b><p>{profileAdvice}</p></div><a href="#services">Choose infrastructure ↓</a></div>
    </div></section>

    <section className="providerBar"><div className="shell"><span>CLOUD PROVIDER</span>{(["AWS", "GCP", "Azure", "NCP", "Vultr"] as Provider[]).map((item) => <button key={item} className={provider === item ? "active" : ""} disabled={item !== "AWS"} onClick={() => setProvider(item)}>{item}{item !== "AWS" && <small>SOON</small>}</button>)}</div></section>
